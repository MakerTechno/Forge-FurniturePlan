...
import nowebsite.maker.furnitureplan.common.block.surfacing.entity.renderer.state.WallShelfBlockEntityRenderState;
import org.jetbrains.annotations.NotNull;
import org.jspecify.annotations.Nullable;

import java.util.List;

public class WallShelfBlockEntityRenderer implements BlockEntityRenderer<@NotNull WallShelfBlockEntity, @NotNull WallShelfBlockEntityRenderState> {
    private static final float SCALE_BLOCK = .26f;
    private static final float SCALE_ITEM = .33f;
    public static final List<Vec3> LOCATIONS_SINGLE = List.of(
        new Vec3(13/16f, 5/16f, 11/16f),
        new Vec3(8/16f, 5/16f, 11/16f),
        new Vec3(3/16f, 5/16f, 11/16f)
    );
    public static final List<Vec3> LOCATIONS_BI = List.of(
        new Vec3(13/16f, 9/16f, 11/16f),
        new Vec3(8/16f, 9/16f, 11/16f),
        new Vec3(3/16f, 9/16f, 11/16f),

        new Vec3(13/16f, 1/16f, 11/16f),
        new Vec3(8/16f, 1/16f, 11/16f),
        new Vec3(3/16f, 1/16f, 11/16f)
    );
    ...
    @Override
    public WallShelfBlockEntityRenderState createRenderState() {
        return new WallShelfBlockEntityRenderState();
    }

    @Override
    public void extractRenderState(@NotNull WallShelfBlockEntity blockEntity, @NotNull WallShelfBlockEntityRenderState state, float partialTicks, Vec3 cameraPosition, ModelFeatureRenderer.@Nullable CrumblingOverlay breakProgress) {
        BlockEntityRenderer.super.extractRenderState(blockEntity, state, partialTicks, cameraPosition, breakProgress);
        state.yRot = blockEntity.getBlockState().getValue(WallShelfBlock.FACING).toYRot() + 180;
        state.isBi = blockEntity.getBlockState().getValue(WallShelfBlock.SHAPE).equals(WallShelfShape.BI);
    ...
    @Override
    public void submit(WallShelfBlockEntityRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera) {
        poseStack.pushPose();
        poseStack.translate(0.5, 0.5, 0.5);
        poseStack.mulPose(Axis.YP.rotationDegrees(-state.yRot));
        poseStack.translate(-0.5, -0.5, -0.5);
        for (int i = 0; i < (state.isBi ? 6 : 3); i++) {
            poseStack.pushPose();
    ...
    private static void submitDisplays(WallShelfBlockEntityRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera, int i) {
        ...
        Vec3 pos = state.isBi ? LOCATIONS_BI.get(i) : LOCATIONS_SINGLE.get(i);
    ...
    private static void submitItem(WallShelfBlockEntityRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int i, ItemStackRenderState item) {
    ...
    private static void submitBlockState(WallShelfBlockEntityRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState camera, BlockModelRenderState block, int i) {
    ...
    @Override
    public AABB getRenderBoundingBox(@NotNull WallShelfBlockEntity blockEntity) {
