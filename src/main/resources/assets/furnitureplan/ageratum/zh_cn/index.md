---
title: "家具计划指南"
navigation:
  title: "家具计划指南"
---

# 家具计划指南

本文档描述当前工作区中已经实现的内容。模组仍处于 Alpha 阶段，源码与注册资源是判断功能是否存在的首要依据。

## 当前版本

1. [项目现状与获取方式](current/01-project-status-and-obtaining.md)
2. [家具分类、材料与放置](current/02-categories-materials-and-placement.md)
3. [座具、桌与立柱](current/03-seating-tables-and-columns.md)
4. [展示架与花架](current/04-display-shelves-and-pot-holders.md)
5. [储物家具](current/05-storage-furniture.md)
6. [餐具、灯光与趣味物品](current/06-tableware-lighting-and-novelty.md)
7. [兼容、调试与已知限制](current/07-compatibility-debugging-and-limitations.md)

## 注意事项

- 目标环境为 Minecraft 26.1.2、NeoForge 26.1.2.30-beta 和 Java 25。
- 指南书需要客户端安装 Ageratum 才会显示在创造模式分页中并打开本文档。
