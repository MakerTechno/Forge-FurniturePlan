...
public class GUIUtil {
    ...
    private static void buildVertex(@NotNull VertexConsumer consumer, @NotNull PoseStack poseStack,float p1, float p2, float u0, float u1, int light, int overlay) {
        consumer.addVertex(poseStack.last().pose(), p1, p2, 0)
            .setColor(-1)
            .setUv(u0, u1)
            .setOverlay(overlay)
    ...
    public static void drawTexturedRect(
        VertexConsumer builder, PoseStack transform, float x, float y, float w,
        float h, float u0, float u1, float v0, float v1, int light, int overlay
    ) {
        buildVertex(builder, transform, x, y + h, u0, v1, light, overlay);
        buildVertex(builder, transform, x + w, y + h, u1, v1, light, overlay);
        buildVertex(builder, transform, x + w, y, u1, v0, light, overlay);
        buildVertex(builder, transform, x, y, u0, v0, light, overlay);
    }
